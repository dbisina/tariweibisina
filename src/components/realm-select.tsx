...
import { startClassicalAudio } from "@/lib/classical-audio";
 ...
 *    that realm.
 *  - Choosing a realm sweeps the seam off-screen, then asks Silence or
 *    Classical before entering the site.
 */

const AUDIO_OPTIONS: { mode: AudioMode; label: string; hint: string }[] = [
  { mode: "silence", label: "Silence", hint: "no sound at all" },
  { mode: "classical", label: "Classical", hint: "a new piece plays every day" },
];
...
export function RealmSelect() {
  ...
  const enter = useCallback(
    (mode: AudioMode) => {
      ...
      if (mode === "classical") startClassicalAudio(); // fire inside the click gesture
      const store = useSiteStore.getState();
      store.setAudioMode(mode);
      store.setRealm(picked);
    },
    [picked]
  );
